...
if [ $GENERATE_REFERENCE == "true" ]; then
    echo "Regenerate reference data for $PREFIX (SCDM erfc)"
    mkdir -p reference
    # SCF, NSCF
...
echo "Test $PREFIX (SCDM erfc) without pools"
...
echo "Test $PREFIX (SCDM erfc) with pools"
...
if [ ! -z "$QE_TEST_SERIAL" ]; then
    echo "Test $PREFIX (SCDM erfc) with serial compilation"
