...
if [ $GENERATE_REFERENCE == "true" ]; then
    echo "Regenerate reference data for $PREFIX (SCDM gaussian)"
    mkdir -p reference
    # SCF, NSCF
...
echo "Test $PREFIX (SCDM gaussian) without pools"
...
echo "Test $PREFIX (SCDM gaussian) with pools"
...
if [ ! -z "$QE_TEST_SERIAL" ]; then
    echo "Test $PREFIX (SCDM gaussian) with serial compilation"
